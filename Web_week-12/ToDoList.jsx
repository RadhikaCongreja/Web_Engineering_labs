...
function ToDoList() {
  ...
  // const [changeType, setChangeType] = useState("Backlog");
  ...
  // the functiona that will change status of task and move to that type list
  function changeStatus(todo, newStatus) {
    const updatedTasks = status.map((item) =>
      item === todo ? { ...item, status: newStatus } : item
    );
    setStatus(updatedTasks);
  }
